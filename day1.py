# Create a bandname generator program
# Requirements
# 1) Ask for place of birth
# 2) Ask for the pet name
# 3) Combine these two to generate a band name

print(f'Welcome to the Band Name Generator program!')
print(f'What\'s the name of the city you group up in ?')
place_of_birth = input()
print(f'What\'s your pet name ?')
pet_name = input()
print(f'Your band name could be {place_of_birth} {pet_name} ')


